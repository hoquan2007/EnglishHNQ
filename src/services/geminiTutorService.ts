...
const DR_HNQ_SYSTEM_PROMPT = `
You are Dr. HNQ, the Master AI Tutor & Central Learning Analytics Director for English HNQ platform.
Your objective:
...
`;
    ...
    const fullPrompt = `${DR_HNQ_SYSTEM_PROMPT}\n\n${contextData}\n\n[CONVERSATION HISTORY]\n${pastContext}\n\nSTUDENT QUESTION: ${userMessage}\n\nDR. HNQ RESPONSE:`;
    ...
    console.warn('Dr. HNQ Gemini API call failed, using fallback:', error);
    return generateOfflineTutorResponse(userMessage, user, analytics);
  }
};
...
const generateOfflineTutorResponse = (
  ...
  return {
    reply: `Chào ${user.name}! Thầy Dr. HNQ luôn theo dõi tiến độ của bạn. Hiện tại bạn đã tích lũy **${user.xp} XP** (Rank ${user.rank.toUpperCase()}) và đạt chuỗi **${user.streak} ngày Streak** liên tục.\n\nĐiểm Shadowing trung bình của bạn đang là **${analytics.overallAccuracy}%**. Bạn có câu hỏi nào về từ vựng hay cấu trúc ngữ pháp cần thầy giải đáp thêm không?`
  };
};
