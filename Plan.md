...
| **2026-07-27** | **KHẮC PHỤC 4 VẤN ĐỀ HỆ THỐNG VÀ CẬP NHẬT GEMINI API KEY:** 1) Xáo trộn ngẫu nhiên vị trí đáp án đúng A, B, C, D cho 100% câu hỏi trong bộ đề 500 bài test (khắc phục lỗi lặp đáp án A); 2) Áp dụng thuật toán Fisher-Yates xáo trộn các khối từ & chữ cái cho game `Sentence Builder` và `Word Unscramble` (không bị xếp sẵn từ trái sang phải); 3) Tích hợp YouTube oEmbed metadata + Gemini AI phân tích đúng nội dung thực tế của URL YouTube tùy chỉnh; 4) Tích hợp sẵn Gemini API Key vào hệ thống tự động gán cho Chatbot & AI Tutor. Build clean 100% & auto-sync GitHub. | **HOÀN THÀNH FIX 4 SỰ CỐ & CẬP NHẬT KEY** |

---
*Dự án Nền tảng Học Tiếng Anh AI English HNQ đã hoàn thành sửa 4 lỗi hệ thống và tích hợp API Key Gemini thành công.*
